...
# Verifies that skills are resolved with correct priority: project > personal > hsk-superpowers
...
# 1. Create in hsk-superpowers location (lowest priority)
mkdir -p "$HOME/.config/opencode/hsk-superpowers/skills/priority-test"
cat > "$HOME/.config/opencode/hsk-superpowers/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
...
if [ -f "$HOME/.config/opencode/hsk-superpowers/skills/priority-test/SKILL.md" ]; then
...
# Test 2: Test that personal overrides hsk-superpowers
echo ""
echo "Test 2: Testing personal > hsk-superpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides hsk-superpowers)"
    ...
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|hsk-superpowers" | head -10
fi

# Test 3: Test that project overrides both personal and hsk-superpowers
echo ""
echo "Test 3: Testing project > personal > hsk-superpowers priority..."
...
# Test 4: Test explicit hsk-superpowers: prefix bypasses priority
echo ""
echo "Test 4: Testing hsk-superpowers: prefix forces hsk-superpowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load hsk-superpowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] hsk-superpowers: prefix correctly forces hsk-superpowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] hsk-superpowers: prefix did not force hsk-superpowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
